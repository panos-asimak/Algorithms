...
in_file.open("GreedyResults.txt");
	...
	//OPEN FILE
	int i = 0; string ch; int ind;int ind2;
	getline(in_file, ch);
    ...
    detailed<<endl<<"With total cost-->"<<cost_pleonektiko<<endl;
    detailed<<endl;
	...
	if(cost_pleonektiko-cost_stochastiko <0 )
		{detailed<<"Best solution out of those two---->Greedy"<<endl<<"With less cost ="<<-1*(cost_pleonektiko-cost_stochastiko) <<endl;}
	else
	{
		detailed<<"Best solution out of those two---->Stochastic"<<endl<<"With less cost ="<<cost_pleonektiko-cost_stochastiko<<endl;}/* code */
