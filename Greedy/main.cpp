...
using namespace std;
	...
	int main()
	{
		...
		{ 
			...
			for (k = 0; k < 10; k++)
				{
				if (TSP_array[SeiraEpanalispis][k] != 0 && TSP_array[SeiraEpanalispis][k] <= MinSeiras && visitedStations[k] == 0) {// vres to MinSeiras ths seiras
					MinSeiras = TSP_array[SeiraEpanalispis][k];
					index_MinSeiras = k;
					}	
				}//for k 

			if(MinSeiras>=30){ // vres costos analoga thn apostash
			...
			if(CountDiadromes== 9)// sthn telikh diadromh gia thn epistrofh kanw ksexwristo if mesa sth while
				...
				else { sum_cost += MinSeiras * costAnaKm + telh_MinSeiras * MinSeiras; }
		...
		detailed<<"------------solution-------------"<<endl<<"Destinations-->";
		for(j=0;j<11;j++)
